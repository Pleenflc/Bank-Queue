...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['queue_numbers'])) {
    ...
    if (!empty($queueNumbers)) {
        ...
        $sql = "DELETE FROM account_queue WHERE queue_number IN ($placeholders)";
...
try {
    $sql = "SELECT 
            ...
            LEFT JOIN service s ON aq.service_id = s.service_id
            ORDER BY 
                CASE 
                    WHEN aq.priority = 'VIP' THEN 1 
                    ELSE 2 
                END, aq.time_created";
    $stmt = $pdo->prepare($sql);
    $stmt->execute();
...
<head>
    ...
    <title>Account Safety Queue Dashboard</title>
...
<div class="container">
    ...
    <div class="content">
        <h1>Account Safety Queue Dashboard</h1>
                ...
                <?php if (!empty($result)): ?>
                    <?php foreach ($result as $row): ?>
                        <tr>
                            ...
                            <td>
                                <a href="account_update.php?queue_number=<?php echo htmlspecialchars($row['queue_number']); ?>" class="action-button">Update</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php else: ?>
