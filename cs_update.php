...
if (isset($_POST['queue_number']) && isset($_POST['priority']) && isset($_POST['status'])) {
    ...
    try {
        $sql = "UPDATE cs_queue SET priority = :new_priority, status = :new_status WHERE queue_number = :queue_number";
...
if (isset($_POST['delete_queue_number'])) {
    ...
    try {
        $sql = "DELETE FROM cs_queue WHERE queue_number = :queue_number";
        $stmt = $pdo->prepare($sql);
...
if (isset($_GET['queue_number'])) {
    ...
    try {
        $sql = "SELECT cq.queue_number, cq.service_id, s.Name AS service_name, cq.priority, cq.status, cq.time_created
                FROM cs_queue cq
                JOIN service s ON cq.service_id = s.service_id
                WHERE cq.queue_number = :queue_number";
        $stmt = $pdo->prepare($sql);
...
<head>
    <meta charset="UTF-8">
    <title>Update Customer Support Queue</title>
...
<div class="container">
    <h1>Update Customer Service Queue</h1>
