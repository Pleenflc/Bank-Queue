...
if (isset($_POST['queue_number']) && isset($_POST['priority']) && isset($_POST['status'])) {
    ...
    try {
        $sql = "UPDATE mobile_queue SET priority = :new_priority, status = :new_status WHERE queue_number = :queue_number";
...
if (isset($_POST['delete_queue_number'])) {
    ...
    try {
        $sql = "DELETE FROM mobile_queue WHERE queue_number = :queue_number";
        $stmt = $pdo->prepare($sql);
...
if (isset($_GET['queue_number'])) {
    ...
    try {
        $sql = "SELECT mq.queue_number, mq.service_id, s.Name AS service_name, mq.priority, mq.status, mq.time_created
                FROM mobile_queue mq
                JOIN service s ON mq.service_id = s.service_id
                WHERE mq.queue_number = :queue_number";
        $stmt = $pdo->prepare($sql);
...
<head>
    <meta charset="UTF-8">
    <title>Update Mobile Banking Queue</title>
...
<div class="container">
    <h1>Update Mobile Banking Queue</h1>
