...
if (isset($_POST['queue_number']) && isset($_POST['priority']) && isset($_POST['status'])) {
    ...
    try {
        $sql = "UPDATE teller_queue SET priority = :new_priority, status = :new_status WHERE queue_number = :queue_number";
...
if (isset($_POST['delete_queue_number'])) {
    ...
    try {
        $sql = "DELETE FROM teller_queue WHERE queue_number = :queue_number";
        $stmt = $pdo->prepare($sql);
...
if (isset($_GET['queue_number'])) {
    ...
    try {
        $sql = "SELECT tq.queue_number, tq.service_id, s.Name AS service_name, tq.priority, tq.status, tq.time_created
                FROM teller_queue tq
                JOIN service s ON tq.service_id = s.service_id
                WHERE tq.queue_number = :queue_number";
        $stmt = $pdo->prepare($sql);
...
<head>
    <meta charset="UTF-8">
    <title>Update Teller Queue</title>
