...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['queue_numbers'])) {
    ...
    if (!empty($queueNumbers)) {
        ...
        $sql = "DELETE FROM cs_queue WHERE queue_number IN ($placeholders)";
...
try {
    $sqlCount = "SELECT COUNT(*) as total, 
                        SUM(CASE WHEN priority = 'VIP' THEN 1 ELSE 0 END) as vip_count,
                        SUM(CASE WHEN priority = 'Non-VIP' THEN 1 ELSE 0 END) as non_vip_count,
    ...
    $stmtCount->execute();
    $visitorData = $stmtCount->fetch(PDO::FETCH_ASSOC);
    $totalVisitors = $visitorData['total'] ?? 0;
    $vipCount = $visitorData['vip_count'] ?? 0;
    $nonVipCount = $visitorData['non_vip_count'] ?? 0;
    $waitingCount = $visitorData['waiting_count'] ?? 0;
    $servingCount = $visitorData['serving_count'] ?? 0;
    $completeCount = $visitorData['complete_count'] ?? 0;

    $sql = "SELECT q.queue_number, q.priority, q.status, q.time_created, s.Name AS service_name
        FROM cs_queue q
        LEFT JOIN service s ON q.service_id = s.service_id
        ORDER BY 
            CASE 
                WHEN q.priority = 'VIP' THEN 1 
                ELSE 2 
            END, q.time_created";
    $stmt = $pdo->prepare($sql);
    $stmt->execute();
...
<head>
    ...
    <title>Customer Support Queue Dashboard</title>
...
<div class="container">
    ...
    <div class="content">
        <h1>Customer Support Queue Dashboard</h1>
                ...
                <?php if (!empty($result)): ?>
                    <?php foreach ($result as $row): ?>
                        <tr>
                            ...
                            <td>
                                <a href="cs_update.php?queue_number=<?php echo htmlspecialchars($row['queue_number']); ?>" class="action-button">Update</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php else: ?>
