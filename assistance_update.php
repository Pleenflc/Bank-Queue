...
if (isset($_POST['queue_number']) && isset($_POST['priority']) && isset($_POST['status'])) {
    ...
    try {
        $sql = "UPDATE assistance_queue SET priority = :new_priority, status = :new_status WHERE queue_number = :queue_number";
...
if (isset($_POST['delete_queue_number'])) {
    ...
    try {
        $sql = "DELETE FROM assistance_queue WHERE queue_number = :queue_number";
        $stmt = $pdo->prepare($sql);
...
if (isset($_GET['queue_number'])) {
    ...
    try {
        $sql = "SELECT asq.queue_number, asq.service_id, s.Name AS service_name, asq.priority, asq.status, asq.time_created
                FROM assistance_queue asq
                JOIN service s ON asq.service_id = s.service_id
                WHERE asq.queue_number = :queue_number";
        $stmt = $pdo->prepare($sql);
...
<head>
    <meta charset="UTF-8">
    <title>Update Friendly Assistance Queue</title>
...
<div class="container">
    <h1>Update Friendly Assistance Queue</h1>
