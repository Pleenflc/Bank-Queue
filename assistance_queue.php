...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['queue_numbers'])) {
    ...
    if (!empty($queueNumbers)) {
        ...
        $sql = "DELETE FROM assistance_queue WHERE queue_number IN ($placeholders)";
...
try {
    $sql = "SELECT 
            ...
            LEFT JOIN service s ON asq.service_id = s.service_id
            ORDER BY 
                CASE 
                    WHEN asq.priority = 'VIP' THEN 1 
                    ELSE 2 
                END, asq.time_created";
    $stmt = $pdo->prepare($sql);
    $stmt->execute();
...
<head>
    ...
    <title>Friendly Assistance Queue Dashboard</title>
...
<div class="container">
    ...
    <div class="content">
        <h1>Friendly Assistance Queue Dashboard</h1>
                ...
                <?php if (!empty($result)): ?>
                    <?php foreach ($result as $row): ?>
                        <tr>
                            ...
                            <td>
                                <a href="assistance_update.php?queue_number=<?php echo htmlspecialchars($row['queue_number']); ?>" class="action-button">Update</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php else: ?>
