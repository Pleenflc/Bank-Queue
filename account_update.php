...
if (isset($_POST['queue_number']) && isset($_POST['priority']) && isset($_POST['status'])) {
    ...
    try {
        $sql = "UPDATE account_queue SET priority = :new_priority, status = :new_status WHERE queue_number = :queue_number";
...
if (isset($_POST['delete_queue_number'])) {
    ...
    try {
        $sql = "DELETE FROM account_queue WHERE queue_number = :queue_number";
        $stmt = $pdo->prepare($sql);
...
if (isset($_GET['queue_number'])) {
    ...
    try {
        $sql = "SELECT aq.queue_number, aq.service_id, s.Name AS service_name, aq.priority, aq.status, aq.time_created
                FROM account_queue aq
                JOIN service s ON aq.service_id = s.service_id
                WHERE aq.queue_number = :queue_number";
        $stmt = $pdo->prepare($sql);
...
<head>
    <meta charset="UTF-8">
    <title>Update Account Safety Queue</title>
...
<div class="container">
    <h1>Update Account Safety Queue</h1>
