...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['queue_numbers'])) {
    ...
    if (!empty($queueNumbers)) {
        ...
        $sql = "DELETE FROM teller_queue WHERE queue_number IN ($placeholders)";
...
try {
    $sql = "SELECT 
            ...
            LEFT JOIN service s ON tq.service_id = s.service_id
            ORDER BY 
                CASE 
                    WHEN tq.priority = 'VIP' THEN 1 
                    ELSE 2 
                END, tq.time_created";
    $stmt = $pdo->prepare($sql);
    $stmt->execute();
...
<head>
    ...
    <title>Teller Queue Dashboard</title>
                ...
                <?php if (!empty($result)): ?>
                    <?php foreach ($result as $row): ?>
                        <tr>
                            ...
                            <td>
                                <a href="teller_update.php?queue_number=<?php echo htmlspecialchars($row['queue_number']); ?>" class="action-button">Update</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php else: ?>
